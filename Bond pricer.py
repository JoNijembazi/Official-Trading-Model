...
def strip_constructor(bond):
    strip_number = bond['Maturity Time']* bond['Compound']
    strips = []

    Maturity_time = 1/bond['Compound']
    for i in range(int(strip_number)):
        strip = {}
        # Coupon STRIPS
        if Maturity_time < bond['Maturity Time']:
            strip['Maturity Time'] = Maturity_time
            strip['Coupon'] = 0
            strip['Price'] = (bond['Coupon']/bond['Compound'] * bond['Face']) - rd.randrange(0,10,1)/100
            strip['Face'] = bond['Coupon']/bond['Compound'] * bond['Face']
            strip['Compound'] = bond['Compound']

            strips.append(strip)
        
        elif Maturity_time == bond['Maturity Time']:
            strip['Maturity Time'] = Maturity_time
            strip['Coupon'] = 0
            strip['Price'] = (bond['Coupon']/bond['Compound'] * bond['Face']) - rd.randrange(0,10,1)/100
            strip['Face'] = bond['Coupon']/bond['Compound'] * bond['Face']
...
for i in Zeros:
    i['Yield'] = Yield_to_Maturity(i['Face'], coupon_rate=i['Coupon'],price=i['Price'], years=i['Maturity'], compound=i['Compound']) 
for i in T_s:
    i['Yield'] = Yield_to_Maturity(par=i['Face'],coupon_rate=i['Coupon'], price=i['Price'], years=i['Maturity'], compound=i['Compound'])


Money_market = on_the_run(Zeros)
